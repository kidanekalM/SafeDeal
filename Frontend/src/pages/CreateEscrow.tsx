...
const CreateEscrow = () => {
  ...
  useEffect(() => {
    if (isDetailed) {
      if (milestones.length === 0) append({ title: '', amount: 0 });
  ...
  const selectUser = (user: SearchUser, type: string) => {
    if (type === 'buyer') { setSelectedBuyer(user); setValue('buyer_id', user.id); }
    else if (type === 'seller') { setSelectedSeller(user); setValue('seller_id', user.id); }
    else { setSelectedCounterparty(user); setValue('counterparty_id', user.id); }
    setSearchResults([]);
  };
  ...
  const nextStep = async () => {
    ...
    if (currentStepId === 'role') fieldsToValidate = ['creator_role', 'isDetailed'];
      ...
      if (creatorRole === 'mediator' && (!selectedBuyer || !selectedSeller)) { toast.error(t('pages.select_buyer_seller', 'Select both parties')); return; }
      if (creatorRole !== 'mediator' && !selectedCounterparty) { toast.error(t('pages.select_counterparty', 'Select counterparty')); return; }
      ...
      const total = milestones.reduce((sum, m) => sum + (Number(m.amount) || 0), 0);
      if (total <= 0) { toast.error(t('pages.milestones_amount_error', 'Invalid total')); return; }
  ...
  const onSubmit = async (data: CreateEscrowForm) => {
    try {
      const payload: any = { creator_role: data.creator_role, amount: Number(data.amount), conditions: data.conditions };
      if (data.isDetailed) {
        payload.jurisdiction = data.jurisdiction; payload.governing_law = data.governing_law; payload.dispute_resolution = data.dispute_resolution;
        if (data.milestones) payload.milestones = data.milestones.map((m, i) => ({ ...m, order_index: i, amount: Number(m.amount) }));
      }
      if (creatorRole === 'mediator') { payload.buyer_id = data.buyer_id; payload.seller_id = data.seller_id; payload.mediator_id = currentUser?.id; }
      else if (creatorRole === 'seller') { payload.buyer_id = data.counterparty_id; payload.seller_id = currentUser?.id; }
      else { payload.buyer_id = currentUser?.id; payload.seller_id = data.counterparty_id; }
      await api.post(`/api/escrows?role=${data.creator_role}`, payload);
      toast.success(t('pages.escrow_created_success', 'Created!')); navigate('/escrows');
    } catch (error: any) { toast.error(t('pages.escrow_create_failed', 'Failed')); }
  };
        ...
        <div className="space-y-8">
          ...
          <div className="max-w-md mx-auto">
            <div className="relative">
              ...
              <input type="text" placeholder={t('pages.search_people_placeholder', 'Search...')} className="input w-full h-14 rounded-2xl pl-12" onChange={e => handleSearch(e.target.value)} />
            </div>
            {searchResults.length > 0 && (
              <div className="mt-4 border rounded-2xl overflow-hidden shadow-xl bg-white">
                {searchResults.map(u => <button key={u.id} onClick={() => selectUser(u, creatorRole === 'mediator' ? 'buyer' : 'counterparty')} className="w-full p-4 hover:bg-gray-50 border-b last:border-0 text-left transition-colors">{u.first_name} {u.last_name}</button>)}
              </div>
            )}
            {(selectedCounterparty || selectedBuyer) && <div className="mt-4 p-4 bg-[#e6f7f4] rounded-2xl border border-[#014d46] font-bold text-center">{(selectedCounterparty || selectedBuyer)?.first_name} {(selectedCounterparty || selectedBuyer)?.last_name}</div>}
          </div>
        </div>
      );
      case 'details': return (
        <div className="space-y-8">
          ...
          <textarea rows={8} {...register('conditions')} className="w-full p-5 border-2 rounded-2xl focus:border-[#014d46] outline-none transition-all" placeholder="..." />
          <div className="relative max-w-xs mx-auto">
            <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
            <input type="number" {...register('amount')} className="input pl-12 h-14 rounded-2xl text-xl font-bold w-full" placeholder="0.00" />
          </div>
        </div>
      );
      case 'milestones': return (
        <div className="space-y-8">
          <div className="flex justify-between items-center"><h2 className="text-2xl font-bold">{t('pages.project_milestones', 'Milestones')}</h2><button type="button" onClick={() => append({ title: '', amount: 0 })} className="btn btn-primary btn-sm rounded-xl">+ {t('pages.add', 'Add')}</button></div>
          <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
            {fields.map((f, i) => (
              <div key={f.id} className="p-5 bg-white border-2 rounded-2xl relative shadow-sm">
                <input placeholder="Title" {...register(`milestones.${i}.title`)} className="w-full mb-2 font-bold border-b outline-none focus:border-[#014d46]" />
                <input type="number" placeholder="Amount" {...register(`milestones.${i}.amount`)} className="w-full text-sm outline-none" />
                {fields.length > 1 && <button type="button" onClick={() => remove(i)} className="absolute top-2 right-2 text-red-500 hover:text-red-700 transition-colors"><Trash2 size={16} /></button>}
              </div>
            ))}
          </div>
        </div>
    ...
    <Layout>
      <div className="max-w-5xl mx-auto py-8 px-4 pb-12">
        <div className="flex items-center justify-between mb-12">
          <button onClick={() => step === 0 ? navigate(-1) : setStep(s => s - 1)} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors"><ChevronLeft size={20} /></button>
          <div className="flex-1 max-w-2xl mx-12 h-1 bg-gray-100 rounded-full relative">
            <div className="absolute left-0 top-0 h-full bg-[#014d46] transition-all duration-500 rounded-full" style={{ width: `${(step / (steps.length - 1)) * 100}%` }} />
          </div>
          <div className="w-10" />
        </div>
        ...
        <div className="mt-12 flex justify-between items-center px-4">
          <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('pages.step_indicator', 'Step')} {step + 1} / {steps.length}</div>
          <div className="flex gap-4">
            {step < steps.length - 1 ? (
              <button onClick={nextStep} className="btn btn-primary px-8 h-14 rounded-2xl font-bold uppercase shadow-xl hover:shadow-2xl transition-all flex items-center gap-2 group">{t('pages.continue', 'Continue')} <span className="group-hover:translate-x-1 transition-transform">→</span></button>
            ) : (
              <button onClick={handleSubmit(onSubmit)} disabled={isSubmitting || !isValid} className="btn btn-primary px-8 h-14 rounded-2xl font-bold uppercase shadow-xl hover:shadow-2xl transition-all flex items-center gap-2">{isSubmitting ? t('pages.launching', 'Launching...') : t('pages.start_secure_escrow_btn', 'Start Deal')}<Check size={20} /></button>
            )}
          </div>
        </div>
      </div>
